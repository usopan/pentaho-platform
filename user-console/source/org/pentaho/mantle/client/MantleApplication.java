 ...
 */
public class MantleApplication implements UserSettingsLoadedEventHandler,
	...
	public void loadApplication() {
		...
		UserSettingsManager.getInstance().getUserSettings(
				new AsyncCallback<JsArray<JsSetting>>() {
					public void onSuccess(JsArray<JsSetting> settings) {
						onUserSettingsLoaded(new UserSettingsLoadedEvent(
								settings));
					}
	...
	public static native void showBusyIndicatorById(String title,
	...
	private static void executeCommand(String commandName,
			JavaScriptObject parameterMap) {
		commandExec.execute(commandName, parameterMap);
	}

	private void addGlassPaneListener(JavaScriptObject obj) {
		GlassPane.getInstance().addGlassPaneListener(
				new GlassPaneNativeListener(obj));
	}

	/**
	 * This method is used by things to show a 'mantle' looking alert dialog
	...
	public void onUserSettingsLoaded(UserSettingsLoadedEvent event) {
		...
		MantleSettingsManager.getInstance().getMantleSettings(
				new AsyncCallback<HashMap<String, String>>() {
					public void onSuccess(HashMap<String, String> result) {
						onMantleSettingsLoaded(new MantleSettingsLoadedEvent(
								result));
					}
	...
	public void onMantleSettingsLoaded(MantleSettingsLoadedEvent event) {
		...
		final boolean showOnlyPerspective = Boolean
				.parseBoolean(Window.Location
						.getParameter("showOnlyPerspective"));
		final String startupPerspective = Window.Location
				.getParameter("startupPerspective");

		mantleRevisionOverride = settings.get("user-console-revision");
		RootPanel.get("pucMenuBar").add(
				MantleXul.getInstance().getMenubar());
		...
		if (showOnlyPerspective && !StringUtils.isEmpty(startupPerspective)) {
			...
			RootPanel.get("pucToolBar").add(
					MantleXul.getInstance().getToolbar());			
				if (showOnlyPerspective && !StringUtils.isEmpty(startupPerspective)) {
		...
		overlayPanel.getElement().getStyle()
				.setProperty("position", "absolute");
		RootPanel.get().add(overlayPanel, 0, 0);

		String showAdvancedFeaturesSetting = settings
				.get("show-advanced-features"); //$NON-NLS-1$ 
		showAdvancedFeatures = showAdvancedFeaturesSetting == null ? showAdvancedFeatures
				: Boolean.parseBoolean(showAdvancedFeaturesSetting);

		String submitOnEnterSetting = settings.get("submit-on-enter-key");
		submitOnEnter = submitOnEnterSetting == null ? submitOnEnter : Boolean
				.parseBoolean(submitOnEnterSetting);

		try {
			String restUrl = GWT.getHostPageBaseURL()
					+ "api/repo/files/canAdminister"; //$NON-NLS-1$
			RequestBuilder requestBuilder = new RequestBuilder(
					RequestBuilder.GET, restUrl);
			requestBuilder.setHeader("If-Modified-Since",
			...
			requestBuilder.sendRequest(null, new RequestCallback() {
				...
				public void onError(Request arg0, Throwable arg1) {
					MessageDialogBox dialogBox = new MessageDialogBox(
							Messages.getString("error"), arg1.getLocalizedMessage(), false, false, true); //$NON-NLS-1$
					dialogBox.center();
				}
				...
				public void onResponseReceived(Request arg0, Response response) {
					...
					try {
						String restUrl2 = GWT.getHostPageBaseURL()
								+ "api/scheduler/canSchedule"; //$NON-NLS-1$
						RequestBuilder requestBuilder2 = new RequestBuilder(
								RequestBuilder.GET, restUrl2);
						requestBuilder2.setHeader("If-Modified-Since",
								"01 Jan 1970 00:00:00 GMT");
						requestBuilder2.sendRequest(null,
								new RequestCallback() {
									@Override
									...
									public void onResponseReceived(
											Request arg0, Response response) {
										Boolean isScheduler = Boolean
												.parseBoolean(response
														.getText());
										SolutionBrowserPanel.getInstance()
												.setScheduler(isScheduler);

										String numStartupURLsSetting = settings
												.get("num-startup-urls");
										if (numStartupURLsSetting != null) {
											int numStartupURLs = Integer
													.parseInt(numStartupURLsSetting); //$NON-NLS-1$
											for (int i = 0; i < numStartupURLs; i++) {
												String url = settings
														.get("startup-url-" + (i + 1)); //$NON-NLS-1$
												String name = settings
														.get("startup-name-" + (i + 1)); //$NON-NLS-1$
												if (url != null
														&& !"".equals(url)) { //$NON-NLS-1$
										...
										if (SolutionBrowserPanel.getInstance()
												.getContentTabPanel()
												.getWidgetCount() > 0) {
											SolutionBrowserPanel.getInstance()
													.getContentTabPanel()
										...
										// startup-url on the URL for the app,
										// wins over settings
										String startupURL = Window.Location
												.getParameter("startup-url"); //$NON-NLS-1$
										if (startupURL != null
												&& !"".equals(startupURL)) { //$NON-NLS-1$
											String title = Window.Location
													.getParameter("name"); //$NON-NLS-1$
											startupURL = URL
													.decodeComponent(startupURL);
											SolutionBrowserPanel
													.getInstance()
													.getContentTabPanel()
													.showNewURLTab(title,
		...
		if (!StringUtils.isEmpty(startupPerspective)) {
			if (PerspectiveManager.getInstance().isLoaded()) {
				PerspectiveManager.getInstance().setPerspective(
						startupPerspective);
			} else {
				EventBusUtil.EVENT_BUS.addHandler(PerspectivesLoadedEvent.TYPE,
						new PerspectivesLoadedEventHandler() {
							public void onPerspectivesLoaded(
									PerspectivesLoadedEvent event) {
								PerspectiveManager.getInstance()
										.setPerspective(startupPerspective);
							}
						});
			}
		}
